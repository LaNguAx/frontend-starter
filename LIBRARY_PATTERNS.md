...
- `public/mockServiceWorker.js` is generated and never hand-edited. After `npm ci`, run the
  audited local CLI with `npm exec --offline -- msw init public` when regeneration is
  required. If local dependencies are unavailable, stop; never download an ad hoc MSW
  version or edit the generated worker.
...
- Custom `VITE_*` values are read through the validated `env` export. Direct
  `import.meta.env.DEV` in `src/main.tsx` is the sole environment-access exception because
  Vite must statically eliminate the mock-only dynamic import from the production bundle.
...
The following project-configuration settings are intentional current constraints:

- `strict: true` keeps the application and build configuration on the complete strictness
  family. Do not disable it to suppress a diagnostic; fix the diagnostic.
- `ignoreDeprecations: "6.0"` in `tsconfig.app.json` acknowledges the TypeScript 6
  deprecation window used by this pinned configuration. Do not remove it based on an older
  compiler's option set.
- `erasableSyntaxOnly: true` prevents TypeScript-only runtime constructs such as enums and
  namespaces. Prefer ordinary JavaScript-compatible constructs that erase cleanly.

These options may postdate an agent's training data. `npm run build` with the repository's
pinned compiler is the authority.
...
- Components that need Redux only render through `renderWithProviders`; components that need
  data-router context render through `renderWithRouterAndProviders` with route objects and
  optional `initialEntries` (`src/tests/test-utils.tsx`). Both create a fresh `makeStore()`
  per call. Use the router-aware helper for `NavLink`, `Outlet`, route errors, and navigation
  hooks.
- `@testing-library/jest-dom` and `@testing-library/user-event` are intentionally absent. Use
  Vitest core matchers and `fireEvent` unless a separately approved dependency change adds
  them.
...
a hand-listed `project:` array). `reactHooks.configs.flat.recommended` and
`reactRefresh.configs.vite` are valid current flat-config presets; do not expand them into
legacy hand-wired rule objects. Lint runs with `--max-warnings 0`.
