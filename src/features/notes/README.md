...
| `components/`             | Feature-private components. Nothing outside the feature may import them (except its own page). Interactive patterns that need real accessibility (dialogs, dropdowns, tooltips…) are built on Radix UI primitives, never hand-rolled — see `DeleteNoteDialog.tsx` (focus trap, Esc, ARIA roles for free).                                                                                                                                                       |
